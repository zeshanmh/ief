...
def set_trt_line(ldata, num_trt=1): 
...
def load_synthetic_data_trt(fold_span = range(5), nsamples = {'train':500, 'valid':300, 'test':200}, \
                        ...
                        seed=0, multiplier= 1., alpha_1_complex = False, per_missing=0., add_feats=0, num_trt=1, sub=False):
    np.random.seed(seed)
    random.seed(seed)
    dset = {}
    for fold in fold_span:
        ...
        for k in ['train','valid','test']:
            ...
            if include_line: 
                ...
                dset[fold][k]['a'] = set_trt(np.copy(ldata))
